...
function ensureCloudinaryConfigured() {
    ...
    if (!cloud_name || !api_key || !api_secret) {
        throw new Error('Cloudinary environment variables are missing! Please set CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in your _backend/.env file and restart the backend server.');
    }
    cloudinary.config({
        cloud_name,
        api_key,
            ...
            (error, result) => {
                if (error || !result) {
                    ...
                    if (message.includes('Invalid Signature') || (error as any)?.http_code === 401) {
                        message = 'Cloudinary authentication failed (Invalid Signature / 401). Please double-check that your CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in _backend/.env match your Cloudinary Dashboard credentials exactly.';
                    }
